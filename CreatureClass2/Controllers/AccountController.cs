       ...
       // [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                House teamHouse = db.Houses.Find(model.House);
                var user = new Team
                {
                    UserName = model.Name,
                    Email = model.Name + model.House + "@user.com",
                    RegisteredAt = DateTime.Today,
                    HouseName = model.House,
                    Score = 10,
                    LocationId = teamHouse.StartLocationId
                };


                var result = await UserManager.CreateAsync(user, "ThisIsLong1!");
                try
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        return RedirectToAction("Questions", "Home", new { score = "10" });
                    }
                    AddErrors(result);
                ...
                {
                    return RedirectToAction("Welcome", "Home", new { msg = "'"+model.Name+ "' is already taken" });
                }
